// This is currently identical to deform.glsl
#[compute]
#version 450
...
float get_height(vec3 vertex) {
    ...
    for (int i = 0; i < num_craters; i++) {
        ...
        float cavity = x * x - 1; // cavity shape

        // craters.data[i*2+1].y is rim width and .z is rim steepness
        float rimX = min(x - 1 - craters.data[i*2+1].y, 0);
        float rim = craters.data[i*2+1].z * rimX * rimX;
